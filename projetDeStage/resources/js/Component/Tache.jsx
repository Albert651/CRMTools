...
export default function Tache({ onNewTaskAdded }) {
  ...
  const filteredTaches = taches.filter((tache) => {
    return (
      ...
      (filters.Intervenant === '' || tache.Intervenant === filters.Intervenant) &&
  ...
  const uniqueIntervenant = [...new Set(taches.map((tache) => tache.Intervenant))];
                ...
                <tbody>
                  {currentTaches.length > 0 ? (
                    currentTaches.map((tache) => (
                      <tr key={tache.id} className={tache.etat === 'Réalisé' ? 'table-success' : tache.etat === 'En retard' ? 'table-danger' : ''}>
                        <td>{tache.etat}</td>
                        <td>{tache.intitule}</td>
                        <td>
                          {tache.Intervenant} 
                          {tache.Intervenant === 'Tâche non effectuée' && (
                        ...
                        <td>{tache.numero_contact || 'N/A'}</td>
                        <td>{tache.numero_employer || 'N/A'}</td>
  ...
  <Modal.Header closeButton className='bg-dark'>
    <Modal.Title className='text-light'>{modalMode === 'add' ? 'Ajout Tâche' : 'Modifier Tâche'}</Modal.Title>
  </Modal.Header>
  <Modal.Body>
      ...
      {modalPage === 2 && (
        <>
          ...
          <Form.Group controlId="numero_contact">
            <Form.Label>Numéro contact</Form.Label>
            <Form.Select
              id="contactSelect"
              name='numero_contact'
              className="form-select"
